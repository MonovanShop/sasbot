let handler = async (m, { isOwner, isAdmin, conn, text, participants, args, command }) => {
  ...
  let mensaje = args.join(' ') || '¡Atención grupo!'
  let encabezado = `╭━━〔 🚨 *MENSAJE GRUPAL* 🚨 〕━━⬣\n┃🥇 *Los Tazos Dorados*\n┃\n┃💬 *Mensaje:* ${mensaje}\n┃\n┃📲 *Menciones:*\n`

  let cuerpo = ''
  for (let mem of participants) {
    let numero = mem.id.split('@')[0]
    let lada = numero.replace(/[^0-9]/g, '').slice(0, 3)
    let bandera = paises[lada] || '🌎'
    cuerpo += `┃${bandera} @${numero}\n`
  }

  let pie = `┃\n╰━━━━━〔 🤖 *Los Tazos Dorados* 〕━━━━⬣`

  let textoFinal = encabezado + cuerpo + pie
